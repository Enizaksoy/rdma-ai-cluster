...
expect "$ "
send "echo 'Versa@123!!' | sudo -S docker run --rm -v /dev/infiniband:/dev/infiniband --net=host --privileged mellanox/tcpdump-rdma tcpdump -i ${device} -c 100 -nn -v 'udp' 2>&1 | grep 'tos 0x'\r"
expect {
    "tos 0x" {
        exp_continue
    }
